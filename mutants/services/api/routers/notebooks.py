...
router = APIRouter()
_ORDERABLE_NOTEBOOK_FIELDS = {"created", "updated", "name"}


def _parse_notebook_order(order_by: str) -> tuple[str, str]:
    chunks = order_by.strip().split()
    field = chunks[0] if chunks else "updated"
    direction = chunks[1] if len(chunks) > 1 else "desc"
    if field not in _ORDERABLE_NOTEBOOK_FIELDS:
        raise HTTPException(
            status_code=400,
            detail="order_by field must be one of: created, updated, name",
        )
    ...
    return field, normalized_direction
...
async def get_notebooks(
    ...
    try:
        order_field, order_direction = _parse_notebook_order(order_by)
        filter_clause = "WHERE archived = $archived" if archived is not None else ""
        ...
        query = f"""
            ...
            FROM notebook
            {filter_clause}
            ORDER BY {order_field} {order_direction.upper()}
        """
        query_params = {"archived": archived} if archived is not None else {}
