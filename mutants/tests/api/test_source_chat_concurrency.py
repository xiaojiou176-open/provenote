...
class _FakeSourceChatGraph:
    ...
    def invoke(self, *, input, config):
        ...
        new_messages.append(AIMessage(content=f"ack:{new_messages[-1].content}"))
        with self._state_lock:
            self._state[thread_id] = new_messages
            result = {"messages": list(self._state[thread_id])}
        with self._critical_lock:
            self._critical_active -= 1
        return result
...
@pytest.mark.asyncio
async def test_source_chat_stream_serializes_same_session_and_avoids_message_loss(
    monkeypatch,
):
    fake_graph = _FakeSourceChatGraph()
    main_thread_id = threading.get_ident()
    reset_session_locks()

    session = SimpleNamespace(model_override=None, save=AsyncMock())
    monkeypatch.setattr(source_chat_router, "source_chat_graph", fake_graph)
    monkeypatch.setattr(
    ...
    async def _send_and_consume(message: str) -> str:
        response = await source_chat_router.send_message_to_source_chat(
            request=source_chat_router.SendMessageRequest(message=message),
            source_id="src-1",
            session_id="s1",
        )
        chunks: list[str] = []
    ...
    try:
        stream_a, stream_b = await asyncio.gather(
            _send_and_consume("A"), _send_and_consume("B")
        )
    finally:
        reset_session_locks()

    assert '"type": "complete"' in stream_a
    assert '"type": "complete"' in stream_b
...
@pytest.mark.asyncio
async def test_source_chat_stream_serializes_checkpoint_critical_section_across_sessions(
    monkeypatch,
):
    fake_graph = _FakeSourceChatGraph()
    reset_session_locks()
    ...
    async def _send_and_consume(session_id: str, message: str) -> str:
        response = await source_chat_router.send_message_to_source_chat(
            request=source_chat_router.SendMessageRequest(message=message),
            source_id="src-1",
    ...
    try:
        stream_a, stream_b = await asyncio.gather(
            _send_and_consume("s1", "A"), _send_and_consume("s2", "B")
        )
    finally:
        reset_session_locks()
