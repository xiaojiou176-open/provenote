...
async def test_sources_service_validate_create_process_update_and_retry_paths(
    ...
    monkeypatch.setattr(
        sources_service_router, "Source", Mock(return_value=source_shell)
    )
    ...
    monkeypatch.setattr(
        sources_service_router.asyncio,
        "to_thread",
        AsyncMock(return_value=_FakeSyncResult(ok=True)),
    )
    ...
    monkeypatch.setattr(
        sources_service_router.asyncio,
        "to_thread",
        AsyncMock(return_value=_FakeSyncResult(ok=False, error_message="bad proc")),
    )
