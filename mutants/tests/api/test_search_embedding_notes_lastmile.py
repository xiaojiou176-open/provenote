...
async def test_search_text_database_error_maps_to_500(
    ...
    assert "Search failed: db unavailable" == exc_info.value.detail
...
async def test_ask_endpoint_wraps_unexpected_errors(
    ...
    assert exc_info.value.status_code == 500
    assert "Ask operation failed: resolver boom" == exc_info.value.detail
...
async def test_ask_simple_wraps_unexpected_errors(
    ...
    assert exc_info.value.status_code == 500
    assert "Ask operation failed: simple boom" == exc_info.value.detail
