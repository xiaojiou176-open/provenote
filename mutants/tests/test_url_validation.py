...
class TestUrlValidation:
    ...
    def test_valid_https_url(self):
        ...
        assert validate_url("https://services.api.openai.com", "openai") is None
        assert validate_url("https://example.com/api", "anthropic") is None
        # Should not raise

    def test_valid_http_url(self):
        """Valid HTTP URLs should pass."""
        assert validate_url("http://example.com", "openai") is None
        # Should not raise
    ...
    def test_localhost_allowed_for_self_hosted(self):
        ...
        assert validate_url("http://localhost:8000", "openai") is None
        assert validate_url("http://127.0.0.1:8000", "azure") is None
        # Should not raise

    def test_localhost_allowed_for_ollama(self):
        """Localhost should be allowed for Ollama provider."""
        assert validate_url("http://localhost:11434", "ollama") is None
        assert validate_url("http://127.0.0.1:11434", "ollama") is None
        # Should not raise
    ...
    def test_private_ip_allowed_for_self_hosted(self):
        ...
        assert validate_url("http://10.0.0.1", "openai") is None
        assert validate_url("http://172.16.0.1:8080", "anthropic") is None
        assert validate_url("http://192.168.1.1", "azure") is None
        # Should not raise

    def test_private_ip_allowed_for_ollama(self):
        """Private IP addresses should be allowed for Ollama provider."""
        assert validate_url("http://192.168.1.100:11434", "ollama") is None
        assert validate_url("http://10.0.0.50:11434", "ollama") is None
        # Should not raise

    def test_loopback_allowed_for_self_hosted(self):
        """Loopback addresses should be allowed for self-hosted scenarios."""
        assert validate_url("http://127.0.0.2", "openai") is None
        # Should not raise
    ...
    def test_ipv6_localhost_allowed(self):
        ...
        assert validate_url("http://[::1]:8000", "openai") is None
        # Should not raise

    def test_empty_url(self):
        """Empty URLs should not raise (handled elsewhere)."""
        assert validate_url("", "openai") is None
    ...
    def test_public_hostnames_allowed(self):
        ...
        assert validate_url("https://services.api.openai.com/v1", "openai") is None
        assert validate_url("https://services.api.anthropic.com", "anthropic") is None
        assert (
            validate_url("https://generativelanguage.googleapis.com", "google") is None
        )
        assert validate_url("https://services.api.groq.com", "groq") is None
        # Should not raise

    def test_azure_specific_urls(self):
        """Azure OpenAI endpoints should be validated."""
        assert validate_url("https://my-resource.openai.azure.com", "azure") is None
        # Localhost is allowed for self-hosted
        assert validate_url("http://localhost:8000", "azure") is None
        # Should not raise

    def test_openai_compatible_urls(self):
        """OpenAI-compatible provider URLs should be validated."""
        assert (
            validate_url("https://services.api.together.xyz", "openai_compatible")
            is None
        )
        # Private IPs are allowed for self-hosted
        assert validate_url("http://192.168.1.1:8080", "openai_compatible") is None
        # Should not raise
    ...
    def test_ipv4_mapped_ipv6_private_allowed(self):
        ...
        assert validate_url("http://[::ffff:192.168.1.1]", "openai") is None
