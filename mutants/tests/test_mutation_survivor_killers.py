...
def test_remove_non_ascii_rejects_delete_control_boundary_runtime():
    assert remove_non_ascii(f"A{chr(128)}B") == "AB"


def test_is_allowed_visible_character_runtime_boundary_matrix():
    assert _is_allowed_visible_character("A") is True
