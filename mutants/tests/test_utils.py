"""
Unit tests for the packages.core.utils module.
...
from packages.core.utils.context_builder import ContextBuilder, ContextConfig
...
class TestTextUtilities:
    ...
    def test_extract_text_content_with_string(self):
        """Extract text should return string input as-is."""
        assert extract_text_content("plain response") == "plain response"

    def test_extract_text_content_with_structured_list(self):
        """Extract text should join text from list entries."""
        content = [
    ...
    def test_extract_text_content_ignores_non_text_items(self):
        """Extract text should skip list entries without text."""
        content = [{"type": "tool_call"}, {"foo": "bar"}, 123]
        assert extract_text_content(content) == ""

    def test_extract_text_content_fallback_to_str_for_unknown_types(self):
        """Extract text should stringify unsupported content types."""
...
class TestTokenUtilities:
    ...
    def test_token_count_network_error_fallback(self):
        """Network/cache download failures should still hit the fallback path."""
        import urllib.error
        from unittest.mock import patch
...
class TestVersionUtilities:
    ...
    def test_get_version_from_github_invalid_url(self):
        ...
        from packages.core.utils.version_utils import get_version_from_github
