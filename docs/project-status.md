...
This page explains the current public status boundary for Notebooklab.

In plain language: Notebooklab already has a strong repo-documented local proof loop, public-ready starter packages for several hosts, and a clearer distribution matrix. It now also has a live Official MCP Registry entry for `notebooklab-mcp`, but that is still not the same thing as claiming a hosted product, official vendor endorsement, or every other external listing being live.
...
| --- | --- | --- | --- | --- | --- |
| Claude Code | yes, official Claude Code MCP docs exist and Anthropic now documents a public discovery surface plus marketplace submission forms | [integrations/claude-code.md](integrations/claude-code.md), [../examples/hosts/claude-code/notebooklab-outcome-bundle/README.md](../examples/hosts/claude-code/notebooklab-outcome-bundle/README.md), and [../examples/hosts/claude-code/DIRECTORY_SUBMISSION.md](../examples/hosts/claude-code/DIRECTORY_SUBMISSION.md) | `public-ready package available` | no | authenticated Anthropic submission, marketplace review, and any additional publisher-side materials Anthropic requires at submission time |
| OpenAI Codex | yes, official Codex MCP docs exist | [integrations/codex.md](integrations/codex.md), [../examples/hosts/codex/notebooklab-outcome-bundle/README.md](../examples/hosts/codex/notebooklab-outcome-bundle/README.md), and [../examples/hosts/codex/PLUGIN_DIRECTORY_SUBMISSION.md](../examples/hosts/codex/PLUGIN_DIRECTORY_SUBMISSION.md) | `public-ready package available` | no | wait for OpenAI to expose or document an official listing path before claiming more |
...
| `notebooklab` CLI | a first-party terminal/operator surface for current outcome objects | a separate distributed product line | [runbooks/operator-cli.md](runbooks/operator-cli.md) |
...
| Official MCP Registry | the official registry now returns a live active entry for `io.github.xiaojiou176-open/notebooklab-mcp` | vendor endorsement, hosted runtime, or a package-backed install guarantee | this page, [distribution.md](distribution.md), [../examples/public-distribution/mcp-registry/README.md](../examples/public-distribution/mcp-registry/README.md), and [mcp.md](mcp.md) |
| plugin / marketplace / directory presence | some official host surfaces are already live for Notebooklab (ClawHub and Official MCP Registry), but broader plugin/directory coverage is not universally live today | shipped listing-live truth on selected lanes only | this page and the host pages' non-claim language |
...
| GitHub homepage field | currently points to the GitHub Pages front door at `https://xiaojiou176-open.github.io/notebooklab/` | decide later whether the long-term homepage should stay on GitHub Pages, move to a custom docs site, or move to a custom domain |
| Release surface | `Notebooklab v1.8.5` is published and reflects the current canonical release line | owner decides later whether to publish newer releases or adjust future release copy/assets |
...
| OpenClaw bundle family | public-ready bundle artifacts exist in the repo and the ClawHub skill page is now live at `https://clawhub.ai/xiaojiou176/notebooklab-mcp-outcome-workflows` | broader OpenClaw bundle/plugin storefront validation remains optional later work |
| Official MCP Registry | the public registry now returns an active `notebooklab-mcp` entry | package-backed install artifact upgrades remain optional follow-up work |
...
This page intentionally does **not** claim that Notebooklab is:
