...
def test_package_override_surfaces_are_explicitly_registered() -> None:
    payload = json.loads(
        (REPO_ROOT / "config/upstream/external-surfaces.json").read_text(
            encoding="utf-8"
        )
    ...
    override_entries = {
        item["path"]: item for item in payload["package_override_surfaces"]
    }

    assert override_entries["apps/web/package.json"]["allowed_keys"] == ["overrides"]
    assert override_entries["evals/promptfoo/package.json"]["allowed_keys"] == [
        "overrides"
    ]


def test_implicit_external_surface_guard_ignores_ci_host_frontend_bootstrap_cache() -> (
    None
):
    guard = (
        REPO_ROOT / "tooling/scripts/ci/check_no_implicit_external_surface.py"
    ).read_text(encoding="utf-8")

    assert "IGNORED_PACKAGE_JSON_PREFIXES" in guard
    assert ".runtime-cache/ci-host/bootstrap/apps-web-node-modules/" in guard
