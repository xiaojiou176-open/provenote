...
def test_distribution_doc_keeps_mcp_registry_boundary() -> None:
    text = _read("docs/distribution.md")
    assert "live Official MCP Registry entry for `notebooklab-mcp`" in text
...
def test_registry_prep_files_exist_for_mcp_registry_lane() -> None:
    ...
    assert example_payload["name"] == "io.github.xiaojiou176-open/notebooklab-mcp"
...
def test_readme_project_status_distribution_and_mcp_keep_registry_truth_in_sync() -> None:
    ...
    assert "live websiteUrl-backed `notebooklab-mcp` entry" in readme
    assert "live Official MCP Registry entry for `notebooklab-mcp`" in project_status
    assert "live Official MCP Registry entry for `notebooklab-mcp`" in distribution
    assert "live websiteUrl-backed entry for `notebooklab-mcp`" in mcp
    assert (
...
def test_pyproject_exposes_public_package_metadata() -> None:
    ...
    assert project["name"] == "notebooklab"
    assert "mcp" in project["keywords"]
    assert "long-context" in project["keywords"]
    assert (
        project["urls"]["Homepage"] == "https://xiaojiou176-open.github.io/notebooklab/"
    )
    assert (
        project["urls"]["Documentation"]
        == "https://xiaojiou176-open.github.io/notebooklab/"
    )
    assert project["urls"]["Source"] == "https://github.com/xiaojiou176-open/notebooklab"
...
def test_pyproject_tracks_public_distribution_artifact_ssot() -> None:
    ...
    assert public_artifact["package_name"] == "notebooklab"
    assert public_artifact["package_registry"] == "pypi"
    assert public_artifact["mcp_name"] == "io.github.xiaojiou176-open/notebooklab-mcp"
    assert "notebooklab-mcp" in public_artifact["entrypoints"]
