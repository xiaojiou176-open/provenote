...
def test_runtime_surfaces_live_teardown_entries_bind_witness_tests() -> None:
    registry = _load_registry()
    ...
    for surface_name in (
        ...
        assert surface["verification_lane"] == ".github/workflows/live-integration.yml"
        assert (
            surface["witness_test_name"]
...
def test_runtime_surfaces_release_proof_and_action_evidence_bind_witness_tests() -> (
    ...
    assert (
        release_proof["verification_lane"] == ".github/workflows/build-and-release.yml"
    )
    assert (
...
def test_live_integration_workflow_exports_live_teardown_witness_artifacts() -> None:
    workflow = (WORKFLOWS_DIR / "live-integration.yml").read_text(encoding="utf-8")
    for expected_path in (
        ".runtime-cache/runs/current/evidence/live-teardown/live-llm.jsonl",
        ".runtime-cache/runs/current/evidence/live-teardown/live-external-web.jsonl",
    ):
        matched_steps = _collect_steps_for_path(workflow, expected_path)
        assert matched_steps, (
            f"Live integration workflow must upload witness artifact for {expected_path}."
        )


def test_performance_benchmark_workflow_exports_benchmark_results_evidence() -> None:
    workflow = (WORKFLOWS_DIR / "test.yml").read_text(encoding="utf-8")
    assert (
        ".runtime-cache/runs/current/evidence/benchmarks/benchmark-results.json"
        in workflow
    ), (
        "Performance benchmark workflow must write benchmark evidence to the canonical runtime surface path."
    )
    matched_steps, non_error_steps = _find_non_error_steps(
        workflow,
        ".runtime-cache/runs/current/evidence/benchmarks/benchmark-results.json",
    )
    assert matched_steps, "Expected benchmark results upload step to exist."
    assert not non_error_steps, (
        "Benchmark results artifact must use if-no-files-found: error. "
        f"Offending steps: {[s.name for s in non_error_steps]}"
    )
