...
def test_claude_and_cursor_bundle_manifests_keep_richer_metadata() -> None:
    claude_payload = json.loads(
        _read(
            "examples/hosts/claude-code/notebooklab-outcome-bundle/.claude-plugin/plugin.json"
        )
    )
    openclaw_claude_payload = json.loads(
        _read(
            "examples/hosts/openclaw/notebooklab-claude-bundle/.claude-plugin/plugin.json"
        )
    )
    openclaw_cursor_payload = json.loads(
        _read(
            "examples/hosts/openclaw/notebooklab-cursor-bundle/.cursor-plugin/plugin.json"
        )
    )
    for payload in (
        claude_payload,
        ...
        assert payload["repository"] == "https://github.com/xiaojiou176-open/notebooklab"
