...
def test_every_migration_has_matching_down_script():
    up_versions = {
        int(path.stem)
    ...
    assert up_versions == down_versions
...
def test_all_migrations_do_not_use_invalid_type_flexible_syntax():
    offenders = []
    for path in MIGRATIONS_DIR.glob("*.surrealql"):
        sql = path.read_text(encoding="utf-8")
        if "TYPE FLEXIBLE" in sql:
...
def test_async_migration_manager_registers_latest_up_and_down():
    manager = AsyncMigrationManager()
    assert len(manager.up_migrations) >= 21
    assert len(manager.down_migrations) == len(manager.up_migrations)
    assert (
        "DEFINE FIELD IF NOT EXISTS created ON TABLE draft"
        in manager.up_migrations[-1].sql
    )
    assert (
        "REMOVE FIELD IF EXISTS created ON TABLE draft"
        in manager.down_migrations[-1].sql
    )
