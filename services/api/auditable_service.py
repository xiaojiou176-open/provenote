...
from packages.core.auditable.schemas import DedupJSON
...
from services.api.evidence_service import (
    _as_claims,
    _as_dedup_entries,
    _as_sections,
    _as_source_paragraphs,
    _repair_with_llm,
    rebuild_auditable_record,
)
...
class AuditableService:
    ...
    async def repair_run_target(
        self,
        run_id: str,
        request: AuditableRepairRequest,
        *,
        target_type: str,
    ) -> AuditableRunResponse:
        existing = await self.get_run(run_id)
        source = await Source.get(existing.source_id)
        if not source:
            raise NotFoundError("Source not found")

        source_record_id = ensure_record_id(source.id or existing.source_id)
        source_paragraphs = _as_source_paragraphs(existing.source_paragraphs)
        sections = _as_sections(existing.sections)
        claims = _as_claims(existing.claims)
        dedup_entries = _as_dedup_entries(existing.dedup_entries)
        dedup_json = existing.dedup_json

        if target_type == "claim":
            if request.target_index >= len(claims):
                raise InvalidInputError("Claim index is out of range")
            repaired_payload = await _repair_with_llm(
                model_id=request.model_id or existing.model_id,
                source_paragraphs=source_paragraphs,
                target_type="claim",
                target_payload=claims[request.target_index].model_dump(),
            )
            claims[request.target_index] = claims[request.target_index].model_validate(
                repaired_payload
            )
        else:
            if request.target_index >= len(sections):
                raise InvalidInputError("Section index is out of range")
            repaired_payload = await _repair_with_llm(
                model_id=request.model_id or existing.model_id,
                source_paragraphs=source_paragraphs,
                target_type="section",
                target_payload=sections[request.target_index].model_dump(),
            )
            sections[request.target_index] = sections[
                request.target_index
            ].model_validate(repaired_payload)

        rebuilt = rebuild_auditable_record(
            title="Auditable Long Text Run",
            source_paragraphs=source_paragraphs,
            claims=claims,
            sections=sections,
            dedup_entries=dedup_entries,
            dedup_json=DedupJSON.model_validate(dedup_json),
            model_id=request.model_id or existing.model_id,
            language=existing.language,
            near_dedup_threshold=existing.near_dedup_threshold,
        )
        created = self._extract_record(
            await repo_create(
                "auditable_run",
