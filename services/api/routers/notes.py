...
from packages.core.application.models import NoteCreate, NoteResponse, NoteUpdate
from packages.core.domain.notebook import Note
...
async def create_note(note_data: NoteCreate):
    ...
    try:
        ...
        if not title and note_data.note_type == "ai" and note_data.content:
            from packages.core.graphs.prompt import graph as prompt_graph

            prompt = "Based on the Note below, please provide a Title for this content, with max 15 words"
            graph = cast(Any, prompt_graph)
            result = cast(
                dict[str, Any],
                await graph.ainvoke(
                    {
        ...
        try:
            ...
            if notebook and note_data.notebook_id:
