...
from packages.core.podcasts.models import EpisodeProfile
...
def _assert_google_provider(provider: str, field_name: str) -> str:
    normalized = _normalize_provider(provider)
    if normalized != SUPPORTED_PROVIDER:
        raise HTTPException(
            status_code=400,
            detail=(
                f"'{field_name}' must be '{SUPPORTED_PROVIDER}' in Gemini-only mode."
            ),
        )
    return normalized
...
async def create_episode_profile(profile_data: EpisodeProfileCreate):
    ...
    try:
        outline_provider = _assert_google_provider(
            profile_data.outline_provider, "outline_provider"
        )
        transcript_provider = _assert_google_provider(
            profile_data.transcript_provider, "transcript_provider"
        )
...
async def update_episode_profile(profile_id: str, profile_data: EpisodeProfileCreate):
    ...
    try:
        ...
        profile.outline_provider = outline_provider
        profile.outline_model = profile_data.outline_model
        profile.transcript_provider = transcript_provider
