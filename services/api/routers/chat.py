...
from packages.core.domain.notebook import ChatSession, Note, Notebook, Source
from packages.core.exceptions import (
    NotFoundError,
)
from packages.core.graphs.chat import graph as chat_graph
    ...
    try:
        ...
        async with session_lock:
            ...
            if isinstance(base_values, MutableMapping):
                state_values: dict[str, Any] = dict(base_values)
            else:
                state_values = {}

            existing_messages = state_values.get("messages") or []
            state_values["messages"] = list(existing_messages)
            ...
            # Execute chat graph in thread to avoid blocking event loop
            graph = cast(Any, chat_graph)
            result = cast(
                dict[str, Any],
...
async def build_context(request: BuildContextRequest):
    ...
    try:
        ...
        try:
            from packages.core.utils import token_count
