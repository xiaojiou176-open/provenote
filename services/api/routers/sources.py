...
from services.api.evidence_service import build_source_processing_report
...
@router.get(
    "/sources/{source_id}/processing-report",
    response_model=SourceProcessingReportResponse,
)
async def get_source_processing_report(source_id: str):
    """Get a human-readable processing report for a source."""
    try:
        return await build_source_processing_report(source_id)
    except HTTPException:
        raise
    except Exception as exc:
        logger.error(
            f"Error fetching processing report for source {source_id}: {str(exc)}"
        )
        raise HTTPException(
            status_code=500,
            detail=f"Error fetching source processing report: {str(exc)}",
        )
...
async def reprocess_source(source_id: str):
    """Canonical reprocess entrypoint for the Source QA console."""
    return await retry_source_processing_service(source_id)
