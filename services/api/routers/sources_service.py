...
async def _process_source_sync(
    ...
    try:
        ...
        if not processed_source.full_text or not processed_source.full_text.strip():
            logger.error(
                "Sync processing finished without persisted full_text source_id={}",
                source.id,
            )
            raise HTTPException(
                status_code=500,
                detail="Processed source is missing full_text",
            )
