...
Business logic lives in services.api.credentials_service.
...
async def list_credentials(
    ...
    try:
        if provider:
            credentials = await Credential.get_by_provider(
                _assert_google_provider(provider)
            )
        else:
...
async def create_credential(request: CreateCredentialRequest):
    ...
    try:
        cred = Credential(
            name=request.name,
            provider=_assert_google_provider(request.provider),
...
async def get_credential(credential_id: str):
    ...
    try:
        ...
        _assert_google_provider(cred.provider)
        models = await cred.get_linked_models()
...
async def test_credential(credential_id: str):
    ...
    result = await svc_test_credential(credential_id)
    success = bool(result.get("success"))
    return {
        "provider": "google" if success else "unknown",
        "success": success,
        "message": (
            "Connection test succeeded."
            if success
            else "Connection test failed. Check provider configuration and server logs."
        ),
    }
