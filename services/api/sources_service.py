...
class SourcesService:
    ...
    def is_source_processing_complete(self, source_id: str) -> bool:
        ...
        Returns True only for terminal states (success/failure) or legacy sources
        without async command tracking. Returns False for in-flight, unknown,
        errored, or unavailable status checks.
        """
        try:
            ...
            logger.warning(
                f"Failed to check source processing completion for {source_id}: {e}"
            )
            return False
