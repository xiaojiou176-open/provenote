...
class PodcastService:
    ...
    async def submit_generation_job(
        ...
        try:
            sanitize_episode_name(episode_name)
            ...
            if draft_id:
                ...
                content = draft.result_markdown

                snapshot = draft.verified_brief_snapshot or {}
                briefing_lines = [f"Verified draft: {draft.title} (v{draft.version})"]
                if snapshot:
                    briefing_lines.append(
                        "Use the verified draft snapshot as the canonical briefing basis."
                    )
                if briefing_suffix:
                    briefing_lines.append(f"Additional instructions: {briefing_suffix}")
                briefing_suffix = "\n\n".join(briefing_lines)
            ...
            # Submit command via command control-plane service
            ...
            if not job_id:
                raise ValueError("Failed to submit podcast command")
