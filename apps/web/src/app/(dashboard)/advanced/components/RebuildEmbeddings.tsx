...
export function RebuildEmbeddings() {
  ...
  const startPolling = (cmdId: string) => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        appLog.error("rebuild-embeddings", "Failed to fetch rebuild status", {
          commandId: cmdId,
          error,
        });
      }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">{t.advanced.rebuildEmbeddings}</CardTitle>
        <CardDescription>{t.advanced.rebuildEmbeddingsDesc}</CardDescription>
      </CardHeader>
        ...
        {!isRebuildActive && (
          <div className="space-y-6">
            ...
            <div className="space-y-3" role="group" aria-labelledby="include-label">
              <span id="include-label" className="text-sm font-medium leading-none">
            ...
            {rebuildMutation.isError && (
              <Alert variant="destructive">
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  {t.advanced.rebuild.failed}:{" "}
                  {(rebuildMutation.error as Error)?.message || t.common.error}
                </AlertDescription>
              </Alert>
            )}
          </div>
