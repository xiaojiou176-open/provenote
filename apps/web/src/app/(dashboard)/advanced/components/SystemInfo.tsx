...
import { getRepositoryHomeUrl } from "@/lib/repo-links";

export function SystemInfo() {
  const { t } = useTranslation();
  const repositoryHomeUrl = getRepositoryHomeUrl();
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        ...
        appLog.error("system-info", "Failed to load config", error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <Card className="p-6">
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          {/* Latest Version */}
          {repositoryHomeUrl && config?.latestVersion && (
            <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-between">
            ...
            {repositoryHomeUrl && config?.hasUpdate ? (
          ...
          {!config?.latestVersion && config?.version && (
            <div className="pt-2 text-xs text-muted-foreground">{t.advanced.updateCheckFailed}</div>
          )}
        </div>
      </div>
    </Card>
