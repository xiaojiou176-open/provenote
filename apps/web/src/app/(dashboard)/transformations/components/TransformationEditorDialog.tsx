...
export function TransformationEditorDialog({
  open,
  onOpenChange,
  transformation,
}: TransformationEditorDialogProps) {
  ...
  const onSubmit = async (data: TransformationFormData) => {
    if (transformation) {
      ...
      queryClient.invalidateQueries({
        queryKey: TRANSFORMATION_QUERY_KEYS.transformation(transformation.id),
      });
    } else {
          ...
          {isEditing && isLoading ? (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4">
                ...
                <p className="text-xs text-muted-foreground mt-3">{t.transformations.promptHint}</p>
              </div>
            </>
          )}
