...
export function ConnectionErrorOverlay({ error, onRetry }: ConnectionErrorOverlayProps) {
  const { t } = useTranslation();
  const [showDetails, setShowDetails] = useState(false);
  const supportDocsUrl = getRepositoryBlobUrl("SUPPORT.md");
  ...
  return (
    <div
      ...
      aria-atomic="true"
    >
      <Card className="ui-card-surface w-full max-w-2xl space-y-6 p-8 shadow-xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold" id="error-title">
              {isApiError ? t.connectionErrors.apiTitle : t.connectionErrors.dbTitle}
            </h1>
              ...
              <div className="space-y-2 text-sm bg-muted p-4 rounded font-mono">
                ...
                {error.details.stack && (
                  <div>
                    ...
                    <pre className="mt-2 overflow-x-auto text-xs">{error.details.stack}</pre>
                  </div>
                )}
              </div>
            </CollapsibleContent>
        ...
        <div className="pt-4 border-t">
          <Button onClick={onRetry} className="ui-primary-cta w-full" size="lg">
