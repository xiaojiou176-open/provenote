...
export function NoteEditorDialog({ open, onOpenChange, notebookId, note }: NoteEditorDialogProps) {
  ...
  const noteIdWithPrefix = note?.id ? (note.id.includes(":") ? note.id : `note:${note.id}`) : "";
  ...
  const onSubmit = async (data: CreateNoteFormData) => {
    if (note) {
      ...
      if (!notebookId) {
        appLog.error("note-editor-dialog", "Cannot create note without notebook_id");
        return;
      }
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      ...
      >
        ...
        <DialogDescription className="sr-only">{t.sources.writeNotePlaceholder}</DialogDescription>
        <form onSubmit={handleSubmit(onSubmit)} className="flex h-full flex-col min-w-0">
          {isEditing && noteLoading ? (
            ...
            <>
              ...
              <div className={cn("flex-1 overflow-y-auto", !isEditorFullscreen && "px-6 py-4")}>
                <Controller
                  control={control}
                  name="content"
                  render={({ field }) => (
          ...
          <div className="border-t px-6 py-4 flex justify-end gap-2">
            ...
            <Button type="submit" disabled={isSaving || (isEditing && noteLoading)}>
              {isSaving
                ? isEditing
                  ? `${t.common.saving}...`
                  : `${t.common.creating}...`
