...
export function SourceTypeStep({
  ...
  return (
    <div className="space-y-6">
      <FormSection title={t.sources.title} description={t.sources.processDescription}>
        <Controller
          control={control}
          name="type"
          render={({ field }) => (
                ...
                <TabsContent key={type.value} value={type.value} className="mt-4">
                  ...
                  {type.value === "link" && (
                    <div>
                      ...
                      />
                      <p className="text-xs text-muted-foreground mt-1">{t.sources.batchUrlHint}</p>
                      {errors.url && (
                  ...
                  {type.value === "text" && (
                    <div>
                      ...
                      {hasHtmlContent && (
                        <div className="ui-status-note-info mb-2 rounded-md border p-2">
                          <p className="ui-status-note-info-text text-sm">
      ...
      {!isBatchMode && (
        <FormSection
          ...
          description={selectedType === "text" ? t.sources.titleRequired : t.sources.titleGenerated}
        >
          <Input
            id="source-title"
            {...register("title")}
        ...
        <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm font-medium">
              {t.sources.batchCount
                .replace("{count}", itemCount.toString())
                .replace(
                  "{type}",
                  selectedType === "link" ? t.sources.addUrl : t.sources.uploadFile,
                )}
            </span>
          </div>
