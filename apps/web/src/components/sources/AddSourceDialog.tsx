...
import { parseAndValidateUrls, SourceTypeStep } from "./steps/SourceTypeStep";

const MAX_BATCH_SIZE = 50;

const resolveUploadedFile = (value: unknown): File | undefined => {
...
const createSourceSchema = z
  ...
  .refine(
    (data) => {
      ...
      if (data.type === "upload") {
        return resolveUploadedFile(data.file) !== undefined;
      }
      return true;
    },
    {
...
export function AddSourceDialog({ open, onOpenChange, defaultNotebookId }: AddSourceDialogProps) {
  ...
  const submitSingleSource = async (data: CreateSourceFormData): Promise<void> => {
    ...
    if (data.type === "upload") {
      const file = resolveUploadedFile(data.file);
      if (!file) {
        return;
      }
  ...
  const submitBatch = async (
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        appLog.error("add-source-dialog", "Failed to create source in batch flow", {
          itemLabel,
          error,
        });
        results.failed++;
  ...
  const onSubmit = async (data: CreateSourceFormData) => {
    try {
      ...
      appLog.error("add-source-dialog", "Failed to create source", error);
      setProcessingStatus({
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="min-w-0">
          ...
          <div className="flex justify-between items-center px-6 py-4 border-t border-border bg-muted">
            ...
            <div className="flex gap-2">
              ...
              {/* Show Done button on final step only */}
              {currentStep === 3 && (
                <Button
                  type="submit"
                  disabled={!currentStepValid || createSource.isPending}
                  aria-busy={createSource.isPending}
                  className="ui-primary-cta min-w-[120px]"
                >
