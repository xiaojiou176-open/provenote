...
export function TemplatesTab() {
  ...
  const languageModelOptions = useMemo(() => modelsByProvider(models, "language"), [models]);
  const ttsModelOptions = useMemo(() => modelsByProvider(models, "text_to_speech"), [models]);
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">{t.podcasts.templatesWorkspaceDesc}</p>
      </div>
        ...
        >
          ...
          <AccordionContent className="text-sm text-muted-foreground">
            <div className="space-y-4">
              <p className="text-muted-foreground/90">{t.podcasts.howTemplatesPowerDesc}</p>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground/80">{t.podcasts.workflowHint}</p>
              </div>
            </div>
          </AccordionContent>
        </AccordionItem>
      ...
      {hasError ? (
        <Alert variant="destructive">
          ...
          <AlertDescription>{t.podcasts.failedToLoadTemplatesDesc}</AlertDescription>
        </Alert>
      ) : null}
