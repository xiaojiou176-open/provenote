...
import { useDeleteSpeakerProfile, useDuplicateSpeakerProfile } from "@/lib/hooks/use-podcasts";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t.podcasts.speakerProfilesDesc}</p>
        </div>
              ...
              <Card key={profile.id} className="shadow-sm">
                <CardHeader className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge variant={usageCount > 0 ? "secondary" : "outline"} className="text-xs">
                      {usageCount > 0
                        ? usageCount === 1
                          ? t.podcasts.usedByCount_one
                          : t.podcasts.usedByCount_other.replace("{count}", usageCount.toString())
                        : t.podcasts.unused}
                    </Badge>
                  </div>
                </CardHeader>
                ...
                <CardContent className="space-y-4 text-sm">
                  <div className="space-y-3">
                    {profile.speakers.map((speaker) => (
                      <div key={speaker.name} className="rounded-md border bg-muted/30 p-3">
                        <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2">
                    <Button variant="ghost" size="sm" onClick={() => setEditProfile(profile)}>
