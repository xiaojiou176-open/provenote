...
const getSTATUS_META = (
  ...
  running: {
    ...
    className: "ui-status-badge-processing",
  },
  processing: {
    label: t.podcasts.processingLabel,
    className: "ui-status-badge-processing",
  ...
  completed: {
    ...
    className: "ui-status-badge-completed",
  },
  failed: {
    label: t.podcasts.failedLabel,
    className: "ui-status-badge-failed",
  },
  error: {
    label: t.podcasts.failedLabel,
    className: "ui-status-badge-failed",
  ...
  pending: {
    ...
    className: "ui-status-badge-pending",
  },
  submitted: {
    label: t.podcasts.pendingLabel,
    className: "ui-status-badge-pending",
  ...
  unknown: {
    ...
    className: "ui-status-badge-neutral",
  },
});
...
function StatusBadge({ status }: { status?: EpisodeStatus | null }) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn("ui-status-badge uppercase tracking-wide text-xs", meta.className)}
    >
      {meta.label}
    </Badge>
  );
...
export function EpisodeCard({ episode, onDelete, deleting, onRetry, retrying }: EpisodeCardProps) {
  ...
  useEffect(() => {
    ...
    const loadProtectedAudio = async () => {
      ...
      try {
        ...
        appLog.error("episode-card", "Unable to load podcast audio", {
  ...
  const createdLabel = distance ? t.podcasts.created.replace("{time}", distance) : null;
  ...
  return (
    <Card className="shadow-sm" data-testid={`podcast-episode-card-${episode.id}`}>
      <CardContent className="space-y-4 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-base font-semibold text-foreground">{episode.name}</h3>
                            ...
                            {episode.speaker_profile?.speakers?.map((speaker, index) => (
                              ...
                              >
                                ...
                                <p className="mt-2 whitespace-pre-wrap text-muted-foreground">
                                  <span className="font-semibold">{t.podcasts.backstory}:</span>{" "}
                                  {speaker.backstory}
                                </p>
                                <p className="mt-2 whitespace-pre-wrap text-muted-foreground">
                                  <span className="font-semibold">{t.podcasts.personality}:</span>{" "}
                                  {speaker.personality}
                                </p>
                              </div>
                            ))}
        ...
        {isFailed && episode.error_message ? (
          <div className="ui-status-panel-failed rounded-md border p-3">
            <p className="ui-status-panel-failed-title text-xs font-medium">
              {t.podcasts.errorDetails}
            </p>
            <p className="ui-status-panel-failed-body mt-1 text-xs whitespace-pre-wrap">
