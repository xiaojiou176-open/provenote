...
export function EpisodesTab() {
  ...
  const { episodes, statusGroups, statusCounts, isLoading, isError, refetch, isFetching } =
    usePodcastEpisodes();
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">{t.podcasts.overviewDesc}</p>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={() => setShowGenerateDialog(true)}>{t.podcasts.generateBtn}</Button>
      ...
      {emptyState ? (
        <div className="rounded-lg border border-dashed bg-muted/30 p-10 text-center">
          <p className="text-sm text-muted-foreground">{t.podcasts.noEpisodesYet}</p>
        </div>
      ) : null}
