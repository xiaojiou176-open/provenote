...
import { useDeleteEpisodeProfile, useDuplicateEpisodeProfile } from "@/lib/hooks/use-podcasts";
...
function findSpeakerSummary(speakerProfiles: SpeakerProfile[], speakerName: string) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{t.podcasts.episodeProfilesDesc}</p>
        </div>
      ...
      {sortedProfiles.length === 0 ? (
        ...
        <div className="space-y-4">
          {sortedProfiles.map((profile) => {
            const speakerSummary = findSpeakerSummary(speakerProfiles, profile.speaker_config);
