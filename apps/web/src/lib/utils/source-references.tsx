import { FileEdit, FileText, Lightbulb } from "lucide-react";
...
export function convertSourceReferences(
  ...
  matches.forEach((match, idx) => {
    ...
    // Preserve surrounding text while excluding brackets that will be displayed in the button label.
    let textEndIndex = match.startIndex;
    if (hasDoubleBracketBefore) {
      textEndIndex = match.startIndex - 2;
    } else if (hasSingleBracketBefore) {
      textEndIndex = match.startIndex - 1;
    }
    if (lastIndex < textEndIndex) {
      parts.push(text.substring(lastIndex, textEndIndex));
    }
...
export function createReferenceLinkComponent(
  ...
  const ReferenceLinkComponent = ({
    ...
    if (href?.startsWith("#ref-")) {
      ...
      const IconComponent =
        type === "source" ? FileText : type === "source_insight" ? Lightbulb : FileEdit; // note
