...
export interface CredentialStatus {
  ...
  source: Record<string, "database" | "environment" | "none">;
  legacy_env_detected: Record<string, boolean>;
  encryption_configured: boolean;
  policy_effective?: Record<
...
export const credentialsApi = {
  ...
  delete: async (
    ...
    if (options?.delete_models) {
      params.delete_models = true;
    }
    if (options?.migrate_to) {
      params.migrate_to = options.migrate_to;
    }
