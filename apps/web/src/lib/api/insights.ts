...
export interface SaveInsightAsNoteRequest {
  notebook_id?: string;
}
...
export const insightsApi = {
  ...
  saveAsNote: async (insightId: string, data: SaveInsightAsNoteRequest) => {
  ...
  getCommandStatus: async (commandId: string) => {
    const response = await apiClient.get<CommandJobStatusResponse>(`/commands/jobs/${commandId}`);
    return response.data;
  },
  ...
  waitForCommand: async (
    ...
    for (let i = 0; i < maxAttempts; i++) {
      try {
        ...
        if (status.status === "failed" || status.status === "canceled") {
          appLog.error("insights-api", "Command failed while polling insight generation", {
            commandId,
        ...
        appLog.error("insights-api", "Error checking command status", {
          commandId,
          error,
        });
    ...
    // Timeout
    appLog.warn("insights-api", "Command polling timed out", { commandId, maxAttempts });
    return false;
  },
};
