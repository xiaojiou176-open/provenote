...
export const embeddingApi = {
  ...
  rebuildEmbeddings: async (
    ...
    const response = await apiClient.post<RebuildEmbeddingsResponse>(
      "/embeddings/rebuild",
      request,
    );
    return response.data;
  ...
  getRebuildStatus: async (commandId: string): Promise<RebuildStatusResponse> => {
    const response = await apiClient.get<RebuildStatusResponse>(
      `/embeddings/rebuild/${commandId}/status`,
    );
    return response.data;
  },
};
