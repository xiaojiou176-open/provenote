...
from packages.core.application.outcome_client_mixin import OutcomeAPIClientMixin
...
_FILENAME_FROM_DISPOSITION_RE = re.compile(r'filename="?(?P<filename>[^";]+)"?')


def _is_sensitive_query_key(key: str) -> bool:
    normalized = key.lower()
    return any(fragment in normalized for fragment in SENSITIVE_QUERY_KEY_FRAGMENTS)
...
def _sanitize_url(url: str) -> str:
    ...
    for key, value in parse_qsl(parsed.query, keep_blank_values=True):
        redacted_query.append((key, "***" if _is_sensitive_query_key(key) else value))
...
class APIClient(OutcomeAPIClientMixin):
    ...
    def __init__(self, base_url: Optional[str] = None):
        ...
        timeout_str = str(settings.api_client_timeout)
        try:
    ...
    def _make_request(
        ...
        try:
            ...
            logger.error(
                "Request error method={} url={} error_type={}",
                method_name,
                safe_url,
                e.__class__.__name__,
            )
            raise ConnectionError(
                f"Failed to connect to API: {method_name} {safe_url} ({e.__class__.__name__})"
            ) from e
        except httpx.HTTPStatusError as e:
            logger.error(
                "HTTP error method={} url={} status_code={} response_body=<redacted>",
                method_name,
                safe_url,
            ...
            raise RuntimeError(
                f"API request failed: {e.response.status_code} - {reason}"
            ) from e
        except Exception as e:
            logger.error(
                "Unexpected API client error method={} url={} error_type={}",
                method_name,
                safe_url,
                e.__class__.__name__,
            )
    ...
    def _make_text_request(
        ...
        """Make HTTP request to the API and return plain text content."""
        url = f"{self.base_url}{endpoint}"
        safe_url = _sanitize_url(url)
        method_name = method.upper()
        request_timeout = timeout if timeout is not None else self.timeout
    ...
    def _make_binary_request(
        ...
        """Make HTTP request to the API and return filename plus raw bytes."""
        url = f"{self.base_url}{endpoint}"
        safe_url = _sanitize_url(url)
        method_name = method.upper()
        request_timeout = timeout if timeout is not None else self.timeout
        ...
        try:
            with httpx.Client(timeout=request_timeout) as client:
                ...
                match = _FILENAME_FROM_DISPOSITION_RE.search(disposition)
    ...
    # Provider policy API methods
    def get_provider_policy(self) -> Union[Dict[Any, Any], List[Dict[Any, Any]]]:
        """Get provider fallback policy."""
        return self._make_request("GET", "/api/providers/policy")

    def update_provider_policy(
        self, **policy
    ) -> Union[Dict[Any, Any], List[Dict[Any, Any]]]:
        """Update provider fallback policy."""
        return self._make_request("PUT", "/api/providers/policy", json=policy)

    def get_provider_bootstrap_diagnostics(
        self,
    ) -> Union[Dict[Any, Any], List[Dict[Any, Any]]]:
        """Get provider/bootstrap readiness diagnostics."""
        return self._make_request("GET", "/api/providers/policy/bootstrap-diagnostics")
