...
from packages.core.ai.models import model_manager
from packages.core.application.command_service import (
    RETRY_POLICY_TRANSACTIONAL,
    CommandService,
)
from packages.core.database.repository import ensure_record_id, repo_insert, repo_query
from packages.core.domain.notebook import Note, Source, SourceInsight
...
from packages.core.utils.chunking import ContentType, chunk_text, detect_content_type
...
def _retry_policy(max_attempts: int = 5, wait_max: int = 60) -> Dict[str, object]:
    policy = dict(RETRY_POLICY_TRANSACTIONAL)
    policy.update(
        {
...
async def embed_note_command(input_data: EmbedNoteInput) -> EmbedNoteOutput:
    ...
    with bind_observability_context(command_id=cmd_id, job_kind="embed_note"):
        try:
...
async def embed_insight_command(input_data: EmbedInsightInput) -> EmbedInsightOutput:
    ...
    with bind_observability_context(command_id=cmd_id, job_kind="embed_insight"):
        try:
...
async def embed_source_command(input_data: EmbedSourceInput) -> EmbedSourceOutput:
    ...
    with bind_observability_context(command_id=cmd_id, job_kind="embed_source"):
        try:
            ...
            if not source.full_text or not source.full_text.strip():
                raise ValueError(
                    f"Source '{input_data.source_id}' has no text to embed"
                )

            logger.debug(
                f"Deleting existing embeddings for source {input_data.source_id}"
            )
            await repo_query(
...
async def create_insight_command(
    ...
    with bind_observability_context(command_id=cmd_id, job_kind="create_insight"):
        try:
            logger.info(
            ...
            await CommandService.submit_command_job(
                ...
                command_name="embed_insight",
                command_args={"insight_id": insight_id},
            )
...
async def rebuild_embeddings_command(
    ...
    with bind_observability_context(command_id=cmd_id, job_kind="rebuild_embeddings"):
        try:
            logger.info("=" * 60)
            ...
            for idx, source_id in enumerate(items["sources"], 1):
                try:
                    await CommandService.submit_command_job(
                        module_name="open_notebook",
                        command_name="embed_source",
                        command_args={"source_id": source_id},
                    )
                    sources_submitted += 1
            ...
            for idx, insight_id in enumerate(items["insights"], 1):
                try:
                    ...
                    logger.error(
                        f"Failed to submit embed_insight for {insight_id}: {e}"
                    )
                    failed_submissions += 1
