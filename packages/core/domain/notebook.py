...
class Source(ObjectModel):
    ...
    async def delete(self) -> bool:
        ...
        if self.asset and self.asset.file_path:
            ...
            uploads_root = Path(UPLOADS_FOLDER).resolve(strict=False)
            if not _is_path_within_directory(resolved_file_path, uploads_root):
                logger.warning(
                    "Blocked file deletion outside uploads for source {}: "
                    "requested_path={} resolved_path={} uploads_root={}. "
    ...
    async def save(self) -> Optional[str]:
        ...
        if self.id and self.content and self.content.strip():
            ...
            command_id = await CommandService.submit_command_job(
                ...
                command_args={"note_id": str(self.id)},
            )
...
async def vector_search(
    ...
    if not keyword:
        raise InvalidInputError("Search keyword cannot be empty")
    try:
        from packages.core.utils.embedding import generate_embedding
