...
Current runtime policy:
    Gemini-only (provider="google") for active model provisioning. The schema
    remains provider-agnostic to preserve historical/migration compatibility.
...
from packages.core.utils.encryption import decrypt_value, encrypt_value
...
class Credential(ObjectModel):
    ...
    account. In current runtime mode, active provisioning is Gemini-only
    (`provider="google"`). Models link to credentials via the credential field.
    """
    ...
    async def get_by_provider(cls, provider: str) -> List["Credential"]:
        ...
        results = await repo_query(
            "SELECT * FROM credential WHERE provider = $provider ORDER BY created ASC",
